enum PaylisherPersonProfiles { never, always, identifiedOnly }

enum PaylisherDataMode { wifi, cellular, any }

class PaylisherConfig {
  final String apiKey;
  var host = 'https://ds.paylisher.com';
  var flushAt = 20;
  ...
  var debug = false;
  var optOut = false;
  var personProfiles = PaylisherPersonProfiles.identifiedOnly;
  ...
  var sessionReplayConfig = PaylisherSessionReplayConfig();

  /// iOS only
  var dataMode = PaylisherDataMode.any;

  /// Enable Surveys
  ///
  /// **Notes:**
  /// - After calling `Paylisher().close()`, surveys will not be rendered until the SDK is re-initialized and the next navigation event occurs.
  /// - You must install `PaylisherObserver` in your app for surveys to display
...
class PaylisherSessionReplayConfig {
...
class PaylisherErrorTrackingConfig {
