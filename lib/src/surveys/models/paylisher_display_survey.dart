...
import 'paylisher_display_open_question.dart';
import 'paylisher_display_link_question.dart';
import 'paylisher_display_rating_question.dart';
import 'paylisher_display_choice_question.dart';
import 'paylisher_display_survey_appearance.dart';
import 'paylisher_display_survey_text_content_type.dart';

/// Main survey model containing metadata and questions
@immutable
class PaylisherDisplaySurvey {
  // Convert back from survey dictionary to a Dart object
  // Native platform model -> Dictionary -> Dart model
  factory PaylisherDisplaySurvey.fromDict(Map<String, dynamic> dict) {
    final questions = (dict['questions'] as List).map((q) {
      ...
      final questionDescriptionContentType =
          PaylisherDisplaySurveyTextContentType.fromInt(questionContentTypeRaw);

      final buttonText = q['buttonText'] as String?;

      switch (type) {
        case 'link':
          return PaylisherDisplayLinkQuestion(
            id: id,
            question: question,
        ...
        case 'rating':
          return PaylisherDisplayRatingQuestion(
            id: id,
            question: question,
            ratingType: PaylisherDisplaySurveyRatingType.fromInt(
                q['ratingType'] as int),
        ...
        case 'single_choice':
          return PaylisherDisplayChoiceQuestion(
            id: id,
            question: question,
        ...
        case 'open':
        default:
          return PaylisherDisplayOpenQuestion(
            id: id,
            question: question,
            description: questionDescription,
            descriptionContentType: questionDescriptionContentType,
    ...
    PaylisherDisplaySurveyAppearance? appearance;
    if (dict['appearance'] != null) {
      ...
      final thankYouMessageDescriptionContentType =
          PaylisherDisplaySurveyTextContentType.fromInt(thankYouContentTypeRaw);

      appearance = PaylisherDisplaySurveyAppearance(
  ...
  final String id;
  final String name;
  final List<PaylisherDisplaySurveyQuestion> questions;
  final PaylisherDisplaySurveyAppearance? appearance;
