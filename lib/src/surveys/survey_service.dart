...
///
/// This service uses the PaylisherObserver to access the current navigation context
/// for displaying surveys. Users must add PaylisherObserver to their navigatorObservers list.
class SurveyService {
  ...
  /// Shows a survey using the PaylisherObserver context
  Future<void> showSurvey(
    ...
    if (_isShowingSurvey) {
      printIfDebug('[Paylisher] A survey is already being displayed');
      return;
    }

    // Use the PaylisherObserver's context to show the survey
    if (PaylisherObserver.currentContext != null) {
      printIfDebug('[Paylisher] Using PaylisherObserver context for survey');
      return _showSurveyWithNavigator(
        survey,
        onShown,
        onResponse,
        onClosed,
        PaylisherObserver.currentContext!,
      );
    }

    // If we can't show the survey, log an error
    printIfDebug(
        '[Paylisher] Cannot show survey: No valid context found. To fix this make sure that you have installed PaylisherObserver correctly in your app.');
  }
  ...
  Future<void> _showSurveyWithNavigator(
    ...
    try {
      ...
      printIfDebug('[Paylisher] Error showing survey: $e');
      _isShowingSurvey = false;
      _currentSurveyContext = null;
    }
  }
  ...
  Widget _buildSurveyWidget(
    ...
    return SurveyBottomSheet(
      ...
      onClosed: onClosed,
      appearance: SurveyAppearance.fromPaylisher(survey.appearance),
    );
  }
