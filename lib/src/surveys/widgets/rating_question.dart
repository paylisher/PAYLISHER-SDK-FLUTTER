...
import '../models/paylisher_display_survey_rating_type.dart';
...
class RatingQuestion extends StatefulWidget {
  const RatingQuestion({
    ...
    this.type = PaylisherDisplaySurveyRatingType.number,
  ...
  final PaylisherDisplaySurveyRatingType type;
...
class _RatingQuestionState extends State<RatingQuestion> {
  ...
  Widget _buildRatingButton(int value) {
    ...
    final range = widget.scaleUpperBound - widget.scaleLowerBound + 1;
    if (widget.type == PaylisherDisplaySurveyRatingType.emoji &&
        (range == 3 || range == 5)) {
      final buttonColor =
