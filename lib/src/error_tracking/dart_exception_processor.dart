...
class DartExceptionProcessor {
  /// Converts Dart error/exception and stack trace to Paylisher exception format
  static Map<String, dynamic> processException({
    ...
    // Extract Paylisher metadata if error is wrapped in PaylisherException
    var mechanismType = 'generic';
    var handled = true;
    var currentError = error;

    if (error is PaylisherException) {
    ...
    if (effectiveStackTrace == null ||
      ...
      isGeneratedStackTrace = true; // Flag to remove top Paylisher frames
    }
    ...
    final frames = hasValidStackTrace
        ? _parseStackTrace(
            ...
            removeTopPaylisherFrames: isGeneratedStackTrace,
          )
        : <Map<String, dynamic>>[];
    ...
    // - Stack trace was generated by Paylisher (not from original exception)
  ...
  /// Determines if a stack frame belongs to Paylisher SDK (just check package for now)
  static bool _isPaylisherFrame(Frame frame) {
    return frame.package == 'paylisher_flutter';
  }
  ...
  /// Parses stack trace into Paylisher format
  ///
  ...
  static List<Map<String, dynamic>> _parseStackTrace(
    ...
    List<String>? inAppExcludes,
    bool inAppByDefault = true,
    bool removeTopPaylisherFrames = false,
  }) {
    final chain = Chain.forTrace(stackTrace);
    final frames = <Map<String, dynamic>>[];

    for (final (index, trace) in chain.traces.indexed) {
      bool skipNextPaylisherFrame = removeTopPaylisherFrames;

      for (final frame in trace.frames) {
        // Skip top Paylisher frames?
        if (skipNextPaylisherFrame) {
          if (_isPaylisherFrame(frame)) {
            continue;
          }
          skipNextPaylisherFrame = false;
        }

        final processedFrame = _convertFrameToPaylisher(
          frame,
          inAppIncludes: inAppIncludes,
          inAppExcludes: inAppExcludes,
          inAppByDefault: inAppByDefault,
  ...
  /// Converts a Frame from stack_trace package to Paylisher format
  static Map<String, dynamic>? _convertFrameToPaylisher(
    Frame frame, {
    List<String>? inAppIncludes,
    List<String>? inAppExcludes,
    bool inAppByDefault = true,
