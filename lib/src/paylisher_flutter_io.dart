...
import 'package:paylisher_flutter/src/surveys/survey_service.dart';
import 'package:paylisher_flutter/src/util/logging.dart';
import 'surveys/models/paylisher_display_survey.dart' as models;
...
class PaylisherFlutterIO extends PaylisherFlutterPlatformInterface {
  ...
  Future<dynamic> _handleMethodCall(MethodCall call) async {
    switch (call.method) {
      ...
      default:
        printIfDebug(
            '[Paylisher] ${call.method} not implemented in PaylisherFlutterPlatformInterface');
        return null;
    }
  }
  ...
  Future<void> showSurvey(Map<String, dynamic> survey) async {
    ...
    final widget = PaylisherFlutterPlatformInterface.instance;
    if (widget is! PaylisherFlutterIO) {
      printIfDebug(
          'Cannot show survey: PaylisherFlutterPlatformInterface instance is not PaylisherFlutterIO');
      return;
    }

    final displaySurvey = models.PaylisherDisplaySurvey.fromDict(survey);

    // Try to show using SurveyService
    // This will work if the user has set up the PaylisherObserver correctly in their app
    await SurveyService().showSurvey(
      ...
      (survey, index, response) async {
        ...
        final nextQuestion = PaylisherSurveyNextQuestion(
  ...
  ///
  @override
  Future<void> setup(PaylisherConfig config) async {
