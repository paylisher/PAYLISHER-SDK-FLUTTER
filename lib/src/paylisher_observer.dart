...
import 'paylisher.dart';

typedef ScreenNameExtractor = String? Function(RouteSettings settings);

/// [PaylisherRouteFilter] allows to filter out routes that should not be tracked.
///
/// By default, only [PageRoute]s are tracked.
typedef PaylisherRouteFilter = bool Function(Route<dynamic>? route);

String? defaultNameExtractor(RouteSettings settings) => settings.name;

bool defaultPaylisherRouteFilter(Route<dynamic>? route) => route is PageRoute;

class PaylisherObserver extends RouteObserver<ModalRoute<dynamic>> {
  PaylisherObserver(
  ...
  /// Clears the current navigation context. Called when Paylisher().close() is called.
  ///
  /// For internal use only. Should not be used by app developers.
  ///
  /// Note: Current limitation - After calling this method, Paylisher will not have a valid BuildContext until
  ...
  final PaylisherRouteFilter _routeFilter;
  ...
  void _sendScreenView(Route<dynamic>? route) {
    ...
    if (_isTrackeableRoute(screenName)) {
      ...
      Paylisher().screen(screenName: screenName);
    }
  }
